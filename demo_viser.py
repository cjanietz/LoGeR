...
def extract_frames_from_video(video_path, output_dir, start_frame, end_frame, stride):
    ...
    if actual_end_frame >= total_frames:
        print(
            f"Warning: end_frame ({actual_end_frame}) is beyond total frames ({total_frames - 1}). Adjusting to last frame."
        )
        actual_end_frame = total_frames - 1

    print(
        f"Extracting frames from {video_path}: start={start_frame}, end={actual_end_frame}, stride={stride}"
    )
    ...
    while True:
        ...
        if (
            ...
            and (current_frame_idx - start_frame) % stride == 0
        ):
...
parser.add_argument(
    "--start_frame", type=int, default=0, help="Start frame for video processing"
)
parser.add_argument(
    "--end_frame",
    type=int,
    default=-1,
    help="End frame for video processing (-1 for last frame)",
)
parser.add_argument(
    "--stride", type=int, default=1, help="Stride for frame extraction/loading"
)
parser.add_argument(
    "--background_mode",
    action="store_true",
...
parser.add_argument(
    "--share", action="store_true", help="Share the viser server with others"
)
parser.add_argument(
    "--conf_threshold",
    type=float,
...
parser.add_argument(
    "--seq_name", type=str, default=None, help="Name of the sequence for saving results"
)
parser.add_argument(
    "--subsample",
    type=int,
    default=2,
    help="Subsample the point cloud for visualization by this factor",
)
parser.add_argument(
    "--video_width", type=int, default=320, help="Width of the video display in the GUI"
)
parser.add_argument(
    "--skip_viser",
    action="store_true",
    help="Skip viser visualization and only run inference",
)
parser.add_argument(
    "--model_name",
    type=str,
...
parser.add_argument(
    ...
    type=list,
    default=None,
    help="Target resolution for input images (shorter side).",
)
parser.add_argument(
    "--window_size",
    type=int,
    default=32,
    help="Window size for non-causal inference (-1 for full sequence).",
)
parser.add_argument(
    "--overlap_size",
    type=int,
    default=3,
    help="Overlap size for sliding window inference.",
)
parser.add_argument(
    "--sim3", action="store_true", help="Use sim3 transformation for TTT."
)
parser.add_argument(
    "--sim3_scale_mode",
    type=str,
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Reset TTT / adapter state every N windows (0 disables).",
)
parser.add_argument(
    "--output_txt", type=str, default=None, help="Output trajectory txt file path."
)
parser.add_argument(
    "--se3",
    action="store_true",
...
parser.add_argument(
    ...
    action="store_true",
    default=True,
    help="Use metric scaling for Pi3X (default: True).",
)
parser.add_argument(
    "--no_pi3x_metric",
    action="store_false",
    dest="pi3x_metric",
    help="Disable metric scaling for Pi3X.",
)
parser.add_argument(
...
parser.add_argument(
    "--warmup",
    action="store_true",
    help="Run a warmup inference pass to trigger torch.compile before timing.",
)
parser.add_argument(
    "--benchmark",
    action="store_true",
    help="Run multiple inference passes and report timing statistics.",
)
parser.add_argument(
    "--device",
    type=str,
    ...
    try:
        ...
        if model_name.startswith("yyfz233/"):
            ...
            model = model.from_pretrained(
                model_name, strict=False if pi3x else True, **model_kwargs
            )
...
def run_core_inference(
    ...
    for i, input_path in enumerate(input_paths):
        ...
        if is_video_file(input_path):
            ...
            image_names_current_input = extract_frames_from_video(
                input_path, temp_dir, start_frame, end_frame, stride
            )
            ...
            image_names_current_input = image_names_current_input[
                start_frame:end_idx:stride
            ]
        else:
            print(
                f"Warning: Input path {input_path} is not a valid video file or directory. Skipping."
            )
    ...
    print(
        f"Loading images from combined inputs ({len(all_image_names)} images found)..."
    )
    # Use load_images_from_paths to load exactly the images we collected
    images_tensor = load_images_from_paths(
        all_image_names, Target_W=target_resolution[0], Target_H=target_resolution[1]
    ).to(device)
    ...
    with (
        torch.no_grad(),
        torch.cuda.amp.autocast(enabled=torch.cuda.is_available(), dtype=dtype),
    ):
        raw_model_predictions = model_obj(images_tensor[None])  # Add batch dimension

    # Post-process predictions
    raw_model_predictions["images"] = images_tensor[None].permute(
        0, 1, 3, 4, 2
    )  # B, S, H, W, C
...
def load_images_from_paths(
    image_paths, PIXEL_LIMIT=255000, Target_W=None, Target_H=None, verbose=True
):
    sources = []
...
def main():
    ...
    target_resolution = (
        args.resolution if args.resolution and len(args.resolution) == 2 else None
    )
    ...
    if args.load:
        ...
        if os.path.isdir(saved_predictions_path):
            if args.seq_name and os.path.exists(
                os.path.join(saved_predictions_path, f"{args.seq_name}.pt")
            ):
                saved_predictions_path = os.path.join(
                    saved_predictions_path, f"{args.seq_name}.pt"
                )
            else:
                saved_predictions_path = os.path.join(
                    saved_predictions_path, "predictions.pt"
                )
        ...
        if os.path.exists(saved_predictions_path):
            ...
            try:
                predictions_dict = torch.load(
                    saved_predictions_path, map_location="cpu", weights_only=False
                )
                ...
                for key, value in predictions_dict.items():
                    if isinstance(value, torch.Tensor):
                        ...
                        value, dict
                    ):  # Handle nested dictionaries (e.g., cam01, cam02)
                        for subkey, subvalue in value.items():
                            if isinstance(subvalue, torch.Tensor):
                                predictions_dict[key][subkey] = subvalue.numpy()
                image_folder_for_sky = (
                    args.input
                )  # Assume first input is the reference for sky mask
            except Exception as e:
                print(
                    f"Error loading {saved_predictions_path}: {e}. Proceeding with inference."
                )
                predictions_dict = None
        else:
            print(
                f"No pre-computed results found at {saved_predictions_path}. Proceeding with inference."
            )
    ...
    if predictions_dict is None:
        ...
        input_paths = [
            p
            for p in [args.input, args.input2, args.input3, args.input4, args.input5]
            if p is not None
        ]
        ...
        for i, input_path in enumerate(input_paths):
            if i > 0:
                input_indices[f"cam{i:02d}"] = len(all_image_names_collected)
            ...
            if is_video_file(input_path):
                ...
                current_frames = extract_frames_from_video(
                    input_path, temp_dir, args.start_frame, args.end_frame, args.stride
                )
                all_image_names_collected.extend(current_frames)
            elif os.path.isdir(input_path):
                current_frames = natsorted(
                    glob.glob(os.path.join(input_path, "*.png"))
                    + glob.glob(os.path.join(input_path, "*.jpg"))
                    + glob.glob(os.path.join(input_path, "*.jpeg"))
                )
                ...
                current_frames = current_frames[
                    args.start_frame : end_idx : args.stride
                ]
                all_image_names_collected.extend(current_frames)
        ...
        image_folder_for_sky = (
            os.path.dirname(all_image_names_collected[0])
            if all_image_names_collected
            else None
        )

        if images_tensor.numel() == 0:
            print(
                "Error: No images were loaded successfully. Check image paths and formats."
            )
            return

        print("Running inference...")
        autocast_context, dtype = get_autocast_context(device)
        ...
        if args.config:
                ...
                forward_kwargs.update(
                    {
                        ...
                        else training_settings.get("window_size", -1),
                        "overlap_size": args.overlap_size
                        if args.overlap_size is not None
                        else training_settings.get("overlap_size", 0),
                        "reset_every": args.reset_every
                        if args.reset_every is not None
                        else training_settings.get("reset_every", 0),
                        "num_iterations": config.get(
                            "num_iterations", 1
                        ),  # Or from training_settings
            ...
            args.window_size
            or args.overlap_size
            or args.sim3
            or args.reset_every is not None
        ):
            forward_kwargs.update(
                {
                    "window_size": args.window_size,
        ...
        if args.benchmark:
            ...
            for run_idx in range(num_runs):
                if device.type == "cuda":
                    torch.cuda.synchronize()
                t_start = time.time()
                with torch.no_grad(), get_autocast_context(device)[0]:
            ...
            std_time = (
                sum((t - avg_time) ** 2 for t in inference_times) / len(inference_times)
            ) ** 0.5
            ...
            with torch.no_grad(), get_autocast_context(device)[0]:
                raw_model_predictions = model(
                    images_tensor[None], **forward_kwargs
                )  # Add batch dimension
            ...
            if not args.warmup:
                print(
                    f"  (Note: First run includes torch.compile overhead. Use --warmup for accurate timing)"
                )
            print(f"{'=' * 50}\n")

        if raw_model_predictions is None:
            raise RuntimeError("Inference did not produce any model predictions.")
        ...
        if args.output_folder:
            ...
            output_filename = (
                f"{seq_name_to_use}.pt" if seq_name_to_use else "predictions.pt"
            )
            ...
            try:
                ...
                torch.save(
                    {k: torch.from_numpy(v) for k, v in predictions_dict.items()},
                    output_path,
                )
    ...
    if (
        ...
        and "camera_poses" in predictions_dict
    ):
        ...
        try:
            ...
            if (
                ...
                and not is_video_file(args.input)
            ):
                ...
                timestamps = _try_load_timestamps_for_images(
                    current_frames, Path(args.input)
                )
            else:
                # Fallback to indices
